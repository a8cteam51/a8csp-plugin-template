...
ruleset blocks tag creation; `template-guard.yml` pins the fragment set, the
header-only `CHANGELOG.md`, and the `1.0.0` spawn seed; and `release.yml` skips
its release job here. Everything is keyed to this
...
7. Resolves npm dependencies afresh (only to releases at least seven days old), re-formats the
   substituted JavaScript, rebuilds the committed assets, and resolves Composer dependencies afresh.
...
- `8890`–`8893` (the wp-env ports, matched only inside their `"port":` and
  ports-table anchors) with a four-port block derived from a
